...
export class ScoutClient {
  ...
  async query<Q extends QueryGenqlSelection>(
    query: Q
  ): Promise<GraphQLResponse<Query, Q>> {
    try {
      ...
      if (exception instanceof GenqlError) {
        ...
        const exceptionData = (exc.data ?? {}) as Partial<
          FieldsSelection<Query, Q>
        >;

        if (this.#shouldReportError(exc.errors)) {
          this.#reportErrorFn(
            new Error(
              `Scout API error - ${Object.keys(query).join("_")} - ${
                exc.message
              }`
            )
          );
  ...
  async mutation<M extends MutationGenqlSelection>(
    ...
    try {
      ...
      if (exception instanceof GenqlError) {
        const exc = exception as GenqlError;
        const exceptionData = (exc.data ?? {}) as Partial<
          FieldsSelection<Mutation, M>
        >;

        if (this.#shouldReportError(exc.errors)) {
          this.#reportErrorFn(
            new Error(
              `Scout API error - ${Object.keys(mutation).join("_")} - ${
                exc.message
              }`
            )
          );
