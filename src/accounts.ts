...
const namespace = z.object({
  ...
  orgname: z
    .string()
    .describe("The name of the org which is also the namespace name"),
...
const namespacePaginatedResponseSchema =
  createPaginatedResponseSchema(namespace);
export type NamespacePaginatedResponse = z.infer<
  typeof namespacePaginatedResponseSchema
>;
//#endregion
...
export class Accounts extends Asset {
  ...
  private async getPersonalNamespace(): Promise<CallToolResult> {
    try {
      ...
      return {
        content: [
          { type: "text", text: `The personal namespace is ${username}` },
        ],
      };
    } catch (error) {
      return {
